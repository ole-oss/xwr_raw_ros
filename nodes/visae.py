...
def vis_azi_ele(args, frame):

        publisher_range = rospy.Publisher('range_data',
                                numpy_msg(RangeOut),
                                queue_size=1)

        if not hasattr(vis_azi_ele, 'radar_cubes'):
            vis_azi_ele.radar_cubes = deque(maxlen=3)
        ...
        vis_azi_ele.radar_cubes.append(radar_cube)
        if len(vis_azi_ele.radar_cubes) < vis_azi_ele.radar_cubes.maxlen:
            return
        radar_cube = np.concatenate(vis_azi_ele.radar_cubes, axis=0)
...
if __name__ == '__main__':
    ...
    subscriber_radar = rospy.Subscriber('radar_data',
                                        ...
                                        lambda frame: vis_azi_ele(args, frame),
                                        queue_size=1)
...
# azimuth_vector = np.linspace(0, 360, 50)
# elevation_vector = np.linspace(-90, 90, 50)
...
# plt.title('2D Azimuth-Elevation Signal Pattern')
